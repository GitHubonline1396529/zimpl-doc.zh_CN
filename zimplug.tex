...
目前仅实现了计算项 \code{t} 的绝对值的函数 \code{vabs(t)}，但其他函数
(如两项的最小值、最大值或一个项的符号函数) 可以类似的方式实现。
...
给定一膳食的集合 $F$ 和一营养素的集合 $N$，
有一张表 $\pi_{n}$ 列举了每一种膳食 $f$ 中营养素 $n$ 的含量。
现给定参数 $\Pi_n$，定义为每种营养素的最低摄入量；
$\Delta_f$ 表示每种膳食可接受的最大份数。
已知每种食物的价格 $c_f$，我们需要找到一个满足所有限制条件且总成本最低的膳食配择方案。
为此，为每个 $f \in F$ 引入一个整型变量 $x_f$，用以表示提供膳食 $f$ 的份数。
使用整型变量是因为只提供整份的食物，也就是说，不能选择提供半个鸡蛋。
则该问题可以被定义为：
