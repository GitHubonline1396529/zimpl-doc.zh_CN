...
\code{+}运算符拼接，例如\code{"Hello " + "Keiken"}将会得到\code{"Hello
  Keiken"}。函数\code{substr(string, begin, length)}可用于提取字符串中的特
定部分。\code{begin}是要使用的第一个字符，计数按照第一个字符从0开始。要提
...
\centering
{\sffamily\small
\begin{tabular}{lp{5cm}p{5cm}} % For Chinese displaystyle.
...
\smallskip
\noindent\emph{filename} 是要读取的文件名。
\emph{template} 是一个用于生成元组的模板字符串。
